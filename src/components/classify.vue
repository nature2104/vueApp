...
export default {
   ...
   methods:{
	    ...
	    getData(){
	    	...
	    	this.$http.get(`api/${this.type}?count=10&start=0`)
	   	    	.then((res)=>{
	   	    		res=res.data;
	   	    		if(res.subjects[0]){
	    ...
	    pullUp(callback){
	    	this.$http.get(`api/${this.type}?count=10&start=${Math.floor(Math.random()*250)}`)
		    	.then((res)=>{
	    ...
	    pullDown(callback){
	    	this.$http.get(`api/${this.type}?count=10&start=${(++this.pullDownPage)*10}`)
		    	.then((res)=>{
	   	    		res=res.data;
	   	    		if(res.subjects[0]){
